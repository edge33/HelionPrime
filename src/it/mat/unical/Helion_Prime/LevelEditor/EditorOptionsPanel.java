...
public class EditorOptionsPanel extends JPanel {
	...
	public EditorOptionsPanel(GridPanel gridPanel,
			MapElementsPanel mapElementsPanel, MainMenuPanel mainMenuPanel,
			WavePanel wavePanel) {
	...
	public void createButton() {
		...
		showfloorButton.setFont(MainMenuFrame.getInstance().getMainMenuPanel()
				.getFont());
		showfloorButton.setFont(showfloorButton.getFont().deriveFont(15.0f));
		showfloorButton.setBorderPainted(false);
		// showfloorButton.setBorder(BorderFactory.createLineBorder(Color.green,
		// 1));

		saveButton.setFont(MainMenuFrame.getInstance().getMainMenuPanel()
				.getFont());
		...
		loadButton.setFont(MainMenuFrame.getInstance().getMainMenuPanel()
				.getFont());
		...
		resetButton.setFont(MainMenuFrame.getInstance().getMainMenuPanel()
				.getFont());
		...
		this.addMouseListener(new MouseListener() {
			...
			public void mouseEntered(MouseEvent e) {
				setCursor(MainMenuFrame.getInstance().getMainMenuPanel()
	...
	public void addListener() {
		...
		backButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					UIManager.setLookAndFeel(OptionsPanel
							.getOriginalLookAndFeel());
					setCursor(MainMenuFrame.getInstance().getMainMenuPanel()
							.createCustomCursor(null));
				...
				EditorOptionsPanel.this.mainMenuFrame
						.switchTo(EditorOptionsPanel.this.mainMenuPanel);
			}
		});
		...
		if (enemy == true && spawn == true && room == true) {
			...
			if (response == JFileChooser.APPROVE_OPTION) {
				try {
					...
					out.println("1000");
					out.println(gridPanel.getLevelStruct().getRow() + " "
							+ gridPanel.getLevelStruct().getColumn());
					for (int i = 0; i < gridPanel.getLevelStruct().getRow(); i++) {
						for (int j = 0; j < gridPanel.getLevelStruct()
								.getColumn(); j++) {
							if (gridPanel.getLevelStruct().getElementAt(i, j) > 9) {
								out.print(gridPanel.getLevelStruct()
										.getElementAt(i, j));
								out.print(" ");
							} else {
								if (gridPanel.getLevelStruct().getElementAt(i,
										j) == 4)
									out.print(0);
								else
									out.print(gridPanel.getLevelStruct()
					...
					JOptionPane.showMessageDialog(null,
							"Impossibile Salvare Livello. Contattare Ricca");
				}
			}
		} else {
			JOptionPane
					.showMessageDialog(null,
							"Mancano alcuni componenti essenziali, aggiungili o contatta Ricca");
		}
	}
	...
	public String takeSnapShot(GridPanel panel, String path, String name) {
		...
		BufferedImage bufImage = new BufferedImage(gridPanel.getCols() * 50,
	...
	public void saveWave(String name) {
		try {
			final PrintWriter out = new PrintWriter(new File("waves/" + name
					+ ".txt"));
